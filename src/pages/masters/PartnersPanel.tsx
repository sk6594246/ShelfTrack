...
export function PartnersPanel() {
  ...
  const roleLabel = (p: BusinessPartner) =>
    p.roles.map((r) => (r === 'supplier' ? 'Supplier' : 'Customer')).join(' · ');
    ...
    <div className="grid gap-4 lg:grid-cols-5">
      <div className="space-y-3 lg:col-span-2">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openCreate} className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-700">
            <Plus className="h-4 w-4" /> Add
          </button>
        </div>
        {list.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">No partners yet</div>
        ) : (
              ...
              <li key={p.id}>
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl border bg-white px-3 py-3 text-left shadow-sm transition hover:border-indigo-200 ${
                    editing?.id === p.id ? 'border-indigo-400 ring-2 ring-indigo-100' : 'border-slate-200'
                  }`}
                >
                  <div className="min-w-0">
                    <p className={'truncate font-semibold text-slate-900'}>{p.name}</p>
                    <p className={'text-xs text-slate-500'}>{roleLabel(p)}</p>
                  </div>
        ...
        {formOpen ? (
          <form onSubmit={handleSave} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <h2 className={'font-semibold text-slate-900'}>{editing ? 'Edit partner' : 'New partner'}</h2>
            ...
            <div className="space-y-3">
              <label className="block">
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
              </label>
              <div className="flex gap-4">
                {(['supplier', 'customer'] as PartnerRole[]).map((r) => (
                  <label key={r} className="flex items-center gap-2 text-sm font-medium text-slate-700">
                    <input type="checkbox" checked={roles.includes(r)} onChange={() => toggleRole(r)} className="rounded border-slate-300 text-indigo-600" />
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block">
                  ...
                  <input value={code} onChange={(e) => setCode(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
                </label>
                <label className="block">
                  <span className="text-xs font-semibold text-slate-500">Phone</span>
                  <input value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
                </label>
              </div>
              <label className="block">
                <span className="text-xs font-semibold text-slate-500">Email</span>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
              </label>
              <label className="block">
                    ...
                    {products.map((p) => (
                      <div key={p.id} className="rounded-lg bg-slate-50 px-2 py-2">
                        <p className="truncate text-sm font-medium text-slate-800">
                          {p.name} <span className="text-xs font-normal text-slate-400">({p.sku})</span>
                        </p>
                        <div className="mt-1 flex gap-3">
                          {roles.map((r) => (
                            <label key={r} className="flex items-center gap-1.5 text-xs text-slate-600">
                              <input
                                type="checkbox"
                                checked={linked.some((x) => x.productId === p.id && x.role === r)}
