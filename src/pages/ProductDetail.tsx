...
export function ProductDetail() {
  ...
  const inLocations = useMemo(() => {
    if (!product) return [];
    const assigned = getAssignedLocationsForProduct(product.id);
    if (assigned.length > 0) return assigned;
    return getLocations();
  }, [product]);

  const outLocations = useMemo(() => {
    if (!product) return [];
    return getFifoLocationsForProduct(product.id);
  ...
  async function handleAdjust(direction: 1 | -1) {
    ...
    try {
      ...
      const reason =
        adjustReason.trim() ||
        (direction > 0 ? 'Manual in @ location' : 'Manual out @ location');

      if (direction > 0) {
        await changeStock(product!.id, change, reason);
        receiveBatch({
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Current: {product.quantity} · location required
            </p>
            ...
            <select value={adjustLocationId} onChange={(e) => setAdjustLocationId(e.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
              ...
              <optgroup label="For Add (assigned / all)">
                {inLocations.map((l) => (
                  <option key={`in-${l.id}`} value={l.id}>
                    {l.code ? `${l.name} (${l.code})` : l.name}
                  </option>
                ))}
              </optgroup>
              {outLocations.length > 0 && (
                <optgroup label="For Remove (FIFO stock)">
                  {outLocations.map((row) => (
                    <option key={`out-${row.location.id}`} value={row.location.id}>
                      {row.location.name} — {row.available} avail
                    </option>
                  ))}
                </optgroup>
              )}
