    ...
    <div className="mx-auto w-full max-w-lg p-4 md:p-6">
      <header className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={() => setShowPrint(true)} className="rounded-lg p-2 text-indigo-600 hover:bg-indigo-50" title="Print QR" aria-label="Print QR">
            <Printer className="h-5 w-5" />
          </button>
          <Link to={`/products/${product.id}/edit`} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100">
            <Edit2 className="h-5 w-5" />
          </Link>
          <button onClick={() => setConfirmDelete(true)} className="rounded-lg p-2 text-red-500 hover:bg-red-50">
      ...
      <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3">
          ...
          <button onClick={() => setShowAdjust(true)} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700">
            Adjust stock
          </button>
        </div>
      ...
      <section className="mb-6">
        <h2 className="mb-2 text-sm font-semibold text-slate-700">Recent movements</h2>
        {movements.length === 0 ? (
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <div className="mt-4 flex items-center justify-center gap-4">
              <button onClick={() => setAdjustQty((q) => Math.max(1, q - 1))} className="rounded-full bg-slate-100 p-2">
              ...
              <button onClick={() => setAdjustQty((q) => q + 1)} className="rounded-full bg-slate-100 p-2">
                <Plus className="h-5 w-5" />
              </button>
            </div>
            <input type="text" placeholder="Reason (optional)" value={adjustReason} onChange={(e) => setAdjustReason(e.target.value)} className="mt-4 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
            <div className="mt-5 grid grid-cols-2 gap-3">
              <button onClick={() => handleAdjust(-1)} className="rounded-xl bg-red-50 py-2.5 font-medium text-red-700 hover:bg-red-100">Remove</button>
              <button onClick={() => handleAdjust(1)} className="rounded-xl bg-green-50 py-2.5 font-medium text-green-700 hover:bg-green-100">Add</button>
            </div>
            <button onClick={() => setShowAdjust(false)} className="mt-3 w-full py-2 text-sm text-slate-500">Cancel</button>
          </div>
        </div>
      )}
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">This will permanently remove <strong>{product.name}</strong> and its history.</p>
            <div className="mt-5 grid grid-cols-2 gap-3">
              <button onClick={() => setConfirmDelete(false)} className="rounded-xl bg-slate-100 py-2.5 font-medium text-slate-700">Cancel</button>
              <button onClick={handleDelete} className="rounded-xl bg-red-600 py-2.5 font-medium text-white">Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showPrint && (
        <QRPrintModal
          ...
          payloadJson={payloadToJson(buildProductQRPayload(product))}
          title={product.name}
