...
export function saveCategory(
  ...
  const skuMode = data.skuMode === 'auto' ? 'auto' : 'manual';
  let skuPrefix: string | undefined;
  if (skuMode === 'auto') {
    const p = String(data.skuPrefix || '')
      .trim()
      .toUpperCase()
      .replace(/[^A-Z0-9]/g, '');
    if (!p) throw new Error('Prefix is required when SKU mode is Auto');
    skuPrefix = p;
    const clash = list.find(
      (c) =>
        c.id !== data.id &&
        c.skuMode === 'auto' &&
        String(c.skuPrefix || '').toUpperCase() === skuPrefix
    );
    if (clash) {
      throw new Error('Prefix "' + skuPrefix + '" already used by category "' + clash.name + '"');
    }
  }
