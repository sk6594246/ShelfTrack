...
import { scheduleWriteThrough, writeThroughAfterPost } from '../lib/liveSync';
...
export function createDocument(type: DocumentType): InventoryDocument {
  ...
  scheduleWriteThrough('createDocument');
  return doc;
}
...
export function removeLine(lineId: string): void {
  ...
  scheduleWriteThrough('removeLine');
}
...
export async function postDocument(id: string): Promise<InventoryDocument> {
  ...
  try {
    ...
    const posted = updateDocument(id, {
      status: 'posted',
      postedAt: new Date().toISOString(),
    });
    void writeThroughAfterPost();
    return posted;
  } finally {
...
export async function reverseDocument(id: string): Promise<InventoryDocument> {
  ...
  try {
    ...
    const reversed = updateDocument(id, {
      status: 'reversed',
      reversedAt: now,
